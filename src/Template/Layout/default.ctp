...
<head>
	...
	<? endif; ?>
	<? if(!empty($authUser) && (empty($users))): ?>
		<style>
				.user_text_<?= $authUser['id'];?>
				{
					color:<?= $authUser['color'];?>;
				}
				.user_bg_<?= $authUser['id'];?>
				{
					background:<?= $this->Color->hextorgba($authUser['color'],0.3);?>;
				}
				.user_bg_<?= $authUser['id'];?>:hover
				{
					background:<?= $this->Color->hextorgba($authser['color'],0.6);?>;
				}
				.user_bg_nohover_<?= $authUser['id'];?>
				{
					background:<?= $this->Color->hextorgba($authUser['color'],0.3);?>;
				}
	...
	?>
<? if(!empty($authUser['background'])): ?>
	<style>
		body{
			background:<?= $authUser['color']; ?> url("<?= '/'.$authUser['background']; ?>");
...
<body>	
		<header class="blur user_bg_nohover_<?= $authUser['id'];?>">
			<nav>
				<ul>
		...
		<footer class="blur user_bg_nohover_<?= $authUser['id'];?>">
